...
if ( ! function_exists( 'foton_mikado_set_options_map_position' ) ) {
	function foton_mikado_set_options_map_position( $map ) {
		$position = 10;
		...
		return apply_filters( 'foton_mikado_filter_options_map_position', $position, $map );
	}
}

if ( ! function_exists( 'foton_mikado_add_admin_page' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassAdminPage
	 */
	function foton_mikado_add_admin_page( $attributes ) {
		$slug  = '';
		$title = '';
		$icon  = '';
		...
		if ( isset( $slug ) && ! empty( $title ) ) {
			$admin_page = new FotonMikadoClassAdminPage( $slug, $title, $icon );
			foton_mikado_framework()->mkdOptions->addAdminPage( $slug, $admin_page );
...
if ( ! function_exists( 'foton_mikado_add_admin_panel' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassPanel
	 */
	function foton_mikado_add_admin_panel( $attributes ) {
		$title           = '';
		$name            = '';
		$dependency      = array();
		$args            = array();
		...
		if ( isset( $page ) && ! empty( $title ) && ! empty( $name ) && foton_mikado_framework()->mkdOptions->adminPageExists( $page ) ) {
			$admin_page = foton_mikado_framework()->mkdOptions->getAdminPage( $page );
			
			if ( is_object( $admin_page ) ) {
				$panel = new FotonMikadoClassPanel( $title, $name, $args, $dependency);
...
if ( ! function_exists( 'foton_mikado_add_admin_container' ) ) {
	/**
	 ...
	 */
	function foton_mikado_add_admin_container( $attributes ) {
		$name            = '';
		$parent          = '';
		$dependency      = array();
		...
		if ( ! empty( $name ) && is_object( $parent ) ) {
			$container = new FotonMikadoClassContainer( $name, $dependency );
...
if ( ! function_exists( 'foton_mikado_add_admin_twitter_button' ) ) {
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassTwitterFramework
	 */
	function foton_mikado_add_admin_twitter_button( $attributes ) {
		$name   = '';
		$parent = '';
		...
		if ( ! empty( $parent ) && ! empty( $name ) ) {
			$field = new FotonMikadoClassTwitterFramework();
...
if ( ! function_exists( 'foton_mikado_add_admin_instagram_button' ) ) {
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassInstagramFramework
	 */
	function foton_mikado_add_admin_instagram_button( $attributes ) {
		$name   = '';
		$parent = '';
		...
		if ( ! empty( $parent ) && ! empty( $name ) ) {
			$field = new FotonMikadoClassInstagramFramework();
...
if ( ! function_exists( 'foton_mikado_add_admin_container_no_style' ) ) {
	/**
	 * Generates container object
	 * $attributes can contain:
	 *      $name - name of the container with which it will be added to parent element
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassContainerNoStyle
	 */
	function foton_mikado_add_admin_container_no_style( $attributes ) {
		$name            = '';
		$parent          = '';
		$args            = array();
		$dependency      = array();
		...
		if ( ! empty( $name ) && is_object( $parent ) ) {
			$container = new FotonMikadoClassContainerNoStyle( $name, $args, $dependency );
			$parent->addChild( $name, $container );
...
if ( ! function_exists( 'foton_mikado_add_admin_group' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassGroup
	 */
	function foton_mikado_add_admin_group( $attributes ) {
		$name        = '';
		$title       = '';
		$description = '';
		$parent      = '';
		...
		if ( ! empty( $name ) && ! empty( $title ) && is_object( $parent ) ) {
			$group = new FotonMikadoClassGroup( $title, $description );
...
if ( ! function_exists( 'foton_mikado_add_admin_row' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassRow
	 */
	function foton_mikado_add_admin_row( $attributes ) {
		$parent = '';
		$next   = false;
		$name   = '';
		...
		if ( is_object( $parent ) ) {
			$row = new FotonMikadoClassRow( $next );
...
if ( ! function_exists( 'foton_mikado_add_admin_field' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassField
	 */
	function foton_mikado_add_admin_field( $attributes ) {
		$type            = '';
		$name            = '';
		$default_value   = '';
		$label           = '';
		...
		if ( ! empty( $parent ) && ! empty( $type ) && ! empty( $name ) ) {
			$field = new FotonMikadoClassField( $type, $name, $default_value, $label, $description, $options, $args, $dependency );
...
if ( ! function_exists( 'foton_mikado_add_admin_section_title' ) ) {
	/**
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassTitle
	 */
	function foton_mikado_add_admin_section_title( $attributes ) {
		$parent = '';
		$name   = '';
		$title  = '';
		...
		if ( is_object( $parent ) && ! empty( $title ) && ! empty( $name ) ) {
			$section_title = new FotonMikadoClassTitle( $name, $title );
...
if ( ! function_exists( 'foton_mikado_add_admin_notice' ) ) {
	/**
	 * Generates FotonMikadoClassNotice object from given parameters
	 ...
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassNotice
	 */
	function foton_mikado_add_admin_notice( $attributes ) {
		$title           = '';
		$description     = '';
		$notice          = '';
		$parent          = '';
		...
		if ( is_object( $parent ) && ! empty( $title ) && ! empty( $notice ) && ! empty( $name ) ) {
			$notice_object = new FotonMikadoClassNotice( $title, $description, $notice);
...
if ( ! function_exists( 'foton_mikado_meta_boxes' ) ) {
	/**
	 * Returns instance of FotonMikadoClassMetaBoxes class
	 *
	 * @return FotonMikadoClassMetaBoxes
	 */
	function foton_mikado_meta_boxes() {
		return foton_mikado_framework()->mkdMetaBoxes;
	}
}
 ...
 */
if ( ! function_exists( 'foton_mikado_create_meta_box' ) ) {
	/**
	 * Adds new meta box
	 *
	 * @param $attributes
	 *
	 * @return bool|FotonMikadoClassMetaBox
	 */
	function foton_mikado_create_meta_box( $attributes ) {
		$scope           = array();
		$title           = '';
		$name            = '';
		...
		if ( ! empty( $scope ) && $title !== '' && $name !== '' ) {
			$meta_box_obj = new FotonMikadoClassMetaBox( $scope, $title, $name );
			foton_mikado_framework()->mkdMetaBoxes->addMetaBox( $name, $meta_box_obj );
...
if ( ! function_exists( 'foton_mikado_create_meta_box_field' ) ) {
	/**
	 ...
	 * @return bool|FotonMikadoClassField
	 */
	function foton_mikado_create_meta_box_field( $attributes ) {
		$type            = '';
		$name            = '';
		$default_value   = '';
		$label           = '';
		...
		if ( ! empty( $parent ) && ! empty( $type ) && ! empty( $name ) ) {
			$field = new FotonMikadoClassMetaField( $type, $name, $default_value, $label, $description, $options, $args, $dependency );
...
if ( ! function_exists( 'foton_mikado_add_multiple_images_field' ) ) {
	/**
	 * Generates meta box field object
	 * $attributes can contain:
	 *      $name - name of the field. This will be name of the option in database
	 ...
	 * @return bool|FotonMikadoClassField
	 */
	function foton_mikado_add_multiple_images_field( $attributes ) {
		$name        = '';
		$label       = '';
		$description = '';
		$parent      = '';
		...
		if ( ! empty( $parent ) && ! empty( $name ) ) {
			$field = new FotonMikadoClassMultipleImages( $name, $label, $description );
...
if ( ! function_exists( 'foton_mikado_get_query_order_by_array' ) ) {
	/**
	 ...
	 * @return array
	 */
	function foton_mikado_get_query_order_by_array( $first_empty = false, $additional_elements = array() ) {
		$orderBy = array();
		
		if ( $first_empty ) {
			$orderBy[''] = esc_html__( 'Default', 'foton' );
		}
		
		$orderBy['date']       = esc_html__( 'Date', 'foton' );
		$orderBy['ID']         = esc_html__( 'ID', 'foton' );
		$orderBy['menu_order'] = esc_html__( 'Menu Order', 'foton' );
		$orderBy['name']       = esc_html__( 'Post Name', 'foton' );
		$orderBy['rand']       = esc_html__( 'Random', 'foton' );
		$orderBy['title']      = esc_html__( 'Title', 'foton' );
...
if ( ! function_exists( 'foton_mikado_get_query_order_array' ) ) {
	/**
	 ...
	 * @return array
	 */
	function foton_mikado_get_query_order_array( $first_empty = false ) {
		$order = array();
		...
		$order['ASC']  = esc_html__( 'ASC', 'foton' );
		$order['DESC'] = esc_html__( 'DESC', 'foton' );
