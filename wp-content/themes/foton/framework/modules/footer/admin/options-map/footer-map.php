<?php

if ( ! function_exists( 'foton_mikado_footer_options_map' ) ) {
	function foton_mikado_footer_options_map() {

		foton_mikado_add_admin_page(
			array(
				'slug'  => '_footer_page',
				'title' => esc_html__( 'Footer', 'foton' ),
				'icon'  => 'fa fa-sort-amount-asc'
			)
		);

		$footer_panel = foton_mikado_add_admin_panel(
			array(
				'title' => esc_html__( 'Footer', 'foton' ),
				'name'  => 'footer',
				'page'  => '_footer_page'
			)
		);

		foton_mikado_add_admin_field(
			array(
				'type'          => 'yesno',
				'name'          => 'footer_in_grid',
				'default_value' => 'yes',
				'label'         => esc_html__( 'Footer in Grid', 'foton' ),
				'description'   => esc_html__( 'Enabling this option will place Footer content in grid', 'foton' ),
				'parent'        => $footer_panel
			)
		);

        foton_mikado_add_admin_field(
            array(
                'type'          => 'yesno',
                'name'          => 'uncovering_footer',
                'default_value' => 'no',
                'label'         => esc_html__( 'Uncovering Footer', 'foton' ),
                'description'   => esc_html__( 'Enabling this option will make Footer gradually appear on scroll', 'foton' ),
                'parent'        => $footer_panel,
            )
        );

        foton_mikado_add_admin_field(
            array(
                'type'          => 'select',
                'name'          => 'footer_skin',
                'default_value' => 'light',
                'label'         => esc_html__( 'Footer Skin', 'foton' ),
                'options'       => array(
                    'light'   => esc_html__( 'Light', 'foton' ),
                    'dark' => esc_html__( 'Dark', 'foton' ),
                ),
                'parent'        => $footer_panel,
            )
        );

		foton_mikado_add_admin_field(
			array(
				'type'          => 'yesno',
				'name'          => 'show_footer_top',
				'default_value' => 'yes',
				'label'         => esc_html__( 'Show Footer Top', 'foton' ),
				'description'   => esc_html__( 'Enabling this option will show Footer Top area', 'foton' ),
				'parent'        => $footer_panel,
			)
		);
		
		$show_footer_top_container = foton_mikado_add_admin_container(
			array(
				'name'       => 'show_footer_top_container',
				'parent'     => $footer_panel,
				'dependency' => array(
					'show' => array(
						'show_footer_top' => 'yes'
					)
				)
			)
		);

		foton_mikado_add_admin_field(
			array(
				'type'          => 'select',
				'name'          => 'footer_top_columns',
				'parent'        => $show_footer_top_container,
				'default_value' => '3 3 3 3',
				'label'         => esc_html__( 'Footer Top Columns', 'foton' ),
				'description'   => esc_html__( 'Choose number of columns for Footer Top area', 'foton' ),
				'options'       => array(
					'12' => '1',
					'6 6' => '2',
					'4 4 4' => '3',
                    '3 3 6' => '3 (25% + 25% + 50%)',
					'3 3 3 3' => '4'
				)
			)
		);

		foton_mikado_add_admin_field(
			array(
				'type'          => 'select',
				'name'          => 'footer_top_columns_alignment',
				'default_value' => 'left',
				'label'         => esc_html__( 'Footer Top Columns Alignment', 'foton' ),
				'description'   => esc_html__( 'Text Alignment in Footer Columns', 'foton' ),
				'options'       => array(
					''       => esc_html__( 'Default', 'foton' ),
					'left'   => esc_html__( 'Left', 'foton' ),
					'center' => esc_html__( 'Center', 'foton' ),
					'right'  => esc_html__( 'Right', 'foton' )
				),
				'parent'        => $show_footer_top_container,
			)
		);

		foton_mikado_add_admin_field(
			array(
				'name'        => 'footer_top_background_color',
				'type'        => 'color',
				'label'       => esc_html__( 'Background Color', 'foton' ),
				'description' => esc_html__( 'Set background color for top footer area', 'foton' ),
				'parent'      => $show_footer_top_container
			)
		);

		foton_mikado_add_admin_field(
			array(
				'type'          => 'yesno',
				'name'          => 'show_footer_bottom',
				'default_value' => 'yes',
				'label'         => esc_html__( 'Show Footer Bottom', 'foton' ),
				'description'   => esc_html__( 'Enabling this option will show Footer Bottom area', 'foton' ),
				'parent'        => $footer_panel,
			)
		);

		$show_footer_bottom_container = foton_mikado_add_admin_container(
			array(
				'name'            => 'show_footer_bottom_container',
				'parent'          => $footer_panel,
				'dependency' => array(
					'show' => array(
						'show_footer_bottom'  => 'yes'
					)
				)
			)
		);

		foton_mikado_add_admin_field(
			array(
				'type'          => 'select',
				'name'          => 'footer_bottom_columns',
				'default_value' => '6 6',
				'label'         => esc_html__( 'Footer Bottom Columns', 'foton' ),
				'description'   => esc_html__( 'Choose number of columns for Footer Bottom area', 'foton' ),
				'options'       => array(
					'12' => '1',
					'6 6' => '2',
					'4 4 4' => '3'
				),
				'parent'        => $show_footer_bottom_container,
			)
		);

		foton_mikado_add_admin_field(
			array(
				'name'        => 'footer_bottom_background_color',
				'type'        => 'color',
				'label'       => esc_html__( 'Background Color', 'foton' ),
				'description' => esc_html__( 'Set background color for bottom footer area', 'foton' ),
				'parent'      => $show_footer_bottom_container
			)
		);
	}

	add_action( 'foton_mikado_action_options_map', 'foton_mikado_footer_options_map', foton_mikado_set_options_map_position( 'footer' ) );
}