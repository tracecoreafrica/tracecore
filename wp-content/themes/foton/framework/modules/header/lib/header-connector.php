...
namespace FotonMikadoNamespace\Modules\Header\Lib;

/**
 * Class FotonMikadoClassHeaderConnector
 *
...
class FotonMikadoClassHeaderConnector {
	...
	public function connect( $config = array() ) {
		do_action( 'foton_mikado_action_pre_header_connect' );
		...
		if ( ! empty( $config['affect_content'] ) ) {
			add_filter( 'foton_mikado_filter_content_elem_style_attr', array( $this, 'contentMarginFilter' ) );
		}
		
		if ( ! empty( $config['affect_title'] ) ) {
			add_filter( 'foton_mikado_filter_title_content_padding', array( $this, 'titlePaddingFilter' ) );
		}
		
		do_action( 'foton_mikado_action_after_header_connect' );
	}
	 ...
	 *
	 * Hooks to foton_mikado_filter_title_content_padding filter
	 *
	 * @return int
	 */
