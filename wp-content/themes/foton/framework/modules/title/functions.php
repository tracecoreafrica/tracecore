...
if ( ! function_exists( 'foton_mikado_include_title_types' ) ) {
	/**
	 * Load's all title types by going through all folders that are placed directly in title types folder
	 */
	function foton_mikado_include_title_types() {
		foreach ( glob( MIKADO_FRAMEWORK_MODULES_ROOT_DIR . '/title/types/*/load.php' ) as $module_load ) {
			include_once $module_load;
		}
	}
	
	add_action('foton_mikado_action_options_map', 'foton_mikado_include_title_types', 1); // 1 is set to just be before title option map init
}
...
if ( ! function_exists( 'foton_mikado_get_title_holder_classes' ) ) {
	/**
	 * Function that adds classes to title holder div
	 */
	function foton_mikado_get_title_holder_classes() {
		$page_id            = foton_mikado_get_page_id();
		$title_type_meta    = foton_mikado_get_meta_field_intersect( 'title_area_type', $page_id );
		$title_type         = ! empty( $title_type_meta ) ? $title_type_meta : 'standard';
		$title_in_grid_meta = foton_mikado_get_meta_field_intersect( 'title_area_in_grid', $page_id );
		$title_img          = foton_mikado_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = foton_mikado_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		$title_vertical_alignment = foton_mikado_get_meta_field_intersect( 'title_area_vertical_alignment', $page_id );
		...
		return implode( ' ', apply_filters( 'foton_mikado_filter_title_holder_classes', $classes ) );
	}
}

if ( ! function_exists( 'foton_mikado_get_title_holder_styles' ) ) {
	/**
	 * Function that adds inline styles to title holder div
	 */
	function foton_mikado_get_title_holder_styles() {
		$page_id              = foton_mikado_get_page_id();
		$title_height         = foton_mikado_get_title_area_height();
		$title_bg_color       = foton_mikado_get_meta_field_intersect( 'title_area_background_color', $page_id );
		$title_image          = foton_mikado_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_image_behavior = foton_mikado_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
...
if ( ! function_exists( 'foton_mikado_get_title_holder_data' ) ) {
	/**
	 * Function that adds data attributes to title holder div
	 */
	function foton_mikado_get_title_holder_data() {
		$page_id            = foton_mikado_get_page_id();
		$title_height       = foton_mikado_get_title_area_height();
		$title_img          = foton_mikado_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = foton_mikado_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		...
		if ( ! empty( $title_img ) && $title_img_behavior === 'parallax-zoom-out' ) {
			$attachment_dimensions = foton_mikado_get_image_dimensions( $title_img );
		...
		return apply_filters( 'foton_mikado_filter_title_holder_data', $data );
	}
}

if ( ! function_exists( 'foton_mikado_get_title_wrapper_styles' ) ) {
	/**
	 * Function that adds inline styles to title wrapper div
	 */
	function foton_mikado_get_title_wrapper_styles() {
		$page_id                  = foton_mikado_get_page_id();
		$title_height             = foton_mikado_get_title_area_height();
		$title_content_padding    = foton_mikado_get_title_content_padding();
		$title_img_behavior       = foton_mikado_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		$title_vertical_alignment = foton_mikado_get_meta_field_intersect( 'title_area_vertical_alignment', $page_id );
...
if ( ! function_exists( 'foton_mikado_get_title_background_image' ) ) {
	/**
	 * Function that return background image data if background image is set
	 */
	function foton_mikado_get_title_background_image() {
		$page_id            = foton_mikado_get_page_id();
		$title_img          = foton_mikado_get_meta_field_intersect( 'title_area_background_image', $page_id );
		$title_img_behavior = foton_mikado_get_meta_field_intersect( 'title_area_background_image_behavior', $page_id );
		...
		if ( ! empty( $title_img ) && $title_img_behavior !== 'hide' ) {
			$image_id = foton_mikado_get_attachment_id_from_url( $title_img );
			$alt      = ! empty( $image_id ) ? get_post_meta( $image_id, '_wp_attachment_image_alt', true ) : '';
			
			$image['src'] = $title_img;
			$image['alt'] = ! empty( $alt ) ? esc_html( $alt ) : esc_html__( 'Image Alt', 'foton' );
		}
...
if ( ! function_exists( 'foton_mikado_get_title_area_height' ) ) {
	/**
	 * Function that returns title area height
	 **/
	function foton_mikado_get_title_area_height() {
		$page_id           = foton_mikado_get_page_id();
		$title_height_meta = foton_mikado_get_meta_field_intersect( 'title_area_height', $page_id );
		$title_height      = ! empty( $title_height_meta ) ? intval( $title_height_meta ) : apply_filters( 'foton_mikado_filter_title_area_default_height_value', 240 );
		
		return apply_filters( 'foton_mikado_filter_title_area_height', $title_height );
	}
}

if ( ! function_exists( 'foton_mikado_get_title_content_padding' ) ) {
	/**
	 * Function that returns title content padding
	 **/
	function foton_mikado_get_title_content_padding() {
		$title_content_padding = apply_filters( 'foton_mikado_filter_title_content_padding', 0 );
		
		return intval( $title_content_padding );
	}
}

if ( ! function_exists( 'foton_mikado_get_title_text' ) ) {
	/**
	 * Function that returns current page title text
	 */
	function foton_mikado_get_title_text() {
		$page_id = foton_mikado_get_page_id();
		...
		if ( ( is_home() && is_front_page() ) || is_singular( 'post' ) ) {
			...
			$title = single_term_title( '', false ) . esc_html__( ' Tag', 'foton' );
		} elseif ( is_date() ) {
			$title = get_the_time( 'F Y' );
		} elseif ( is_author() ) {
			$title = esc_html__( 'Author:', 'foton' ) . " " . get_the_author();
		} elseif ( is_category() ) {
			$title = single_cat_title( '', false );
		} elseif ( is_archive() ) {
			$title = esc_html__( 'Archive', 'foton' );
		} elseif ( is_search() ) {
			$title = esc_html__( 'Search results for: ', 'foton' ) . get_search_query();
		} elseif ( is_404() ) {
			$title_404 = foton_mikado_options()->getOptionValue( '404_title' );
			$title     = ! empty( $title_404 ) ? $title_404 : esc_html__( '404 - Page not found', 'foton' );
		}
		
		return apply_filters( 'foton_mikado_filter_title_text', $title );
	}
}

if ( ! function_exists( 'foton_mikado_get_title_styles' ) ) {
	/**
	 * Function that adds inline styles to page title
	 */
	function foton_mikado_get_title_styles() {
		$page_id = foton_mikado_get_page_id();
		$color   = get_post_meta( $page_id, 'mkdf_title_text_color_meta', true );
        $width = get_post_meta($page_id, 'mkdf_title_area_title_width_meta', true);
...
if ( ! function_exists( 'foton_mikado_subtitle_text' ) ) {
	/**
	 * Function that echoes subtitle text.
	 */
	function foton_mikado_subtitle_text() {
		$page_id       = foton_mikado_get_page_id();
		$subtitle_meta = get_post_meta( $page_id, 'mkdf_title_area_subtitle_meta', true );
		$subtitle      = ! empty( $subtitle_meta ) ? $subtitle_meta : '';
		
		return apply_filters( 'foton_mikado_filter_subtitle_title_text', $subtitle );
	}
}

if ( ! function_exists( 'foton_mikado_get_subtitle_styles' ) ) {
	/**
	 * Function that adds inline styles to page subtitle
	 */
	function foton_mikado_get_subtitle_styles() {
		$page_id      = foton_mikado_get_page_id();
        ...
        $width = get_post_meta($page_id, 'mkdf_title_area_subtitle_width_meta', true);
		...
		if ( $side_padding !== '' ) {
			if ( foton_mikado_string_ends_with( $side_padding, '%' ) || foton_mikado_string_ends_with( $side_padding, 'px' ) ) {
